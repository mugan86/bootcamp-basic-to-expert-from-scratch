...
def add_employees_list():
    """ 1.- Introducir número de empleados con validación """
    people_work = ask_employees_amount()
    ...
    for i in range(people_work):
        ...
        employee['category'] = input(
            "Introduce categoría A, B, C, D: ").upper()
        ...
        while True:
            try:
                employee['hours_work'] = int(
                    input("Introduce horas trabajadas: "))
                break
            except ValueError:
                print("Hay que introducir bien las horas de trabajo. Números enteros")
        employee['salary'] = employee['salary_per_hour'] * \
            employee['hours_work']
        employees_list.append(employee)

def classify_by_salary():
    """
